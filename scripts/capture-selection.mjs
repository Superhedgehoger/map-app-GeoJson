...
await page.addInitScript((data) => {
  window.localStorage.removeItem('geomap.workspace.v2');
...
await page.locator('.selection-scenario-report').scrollIntoViewIfNeeded();
...
await page.evaluate(() => {
  document
    .querySelectorAll(
      '[class*="toast"], [class*="brief-message"], #briefMessage, #_briefMsg, #selectionModeHint'
    )
    .forEach((element) => element.remove());
  document.querySelector('.selection-workspace')?.scrollTo({ top: 10000 });
});
await page.waitForTimeout(400);

await mkdir(resolve('docs/images'), { recursive: true });
await page.screenshot({ path: resolve('docs/images/geomap-selection.png') });
await browser.close();
console.log('Captured docs/images/geomap-selection.png (1440x900).');
