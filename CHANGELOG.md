...
## [v4.1.0-beta.2] - 2026-08-23

### 经营事项与行动跟踪

- 根据产品反馈移除决策提交、批准、驳回和撤回相关界面、API、类型与自动化测试
- 新增经营事项：支持关联门店/候选点、责任人、截止日期、说明和四档优先级
- 事项采用待处理、进行中、已完成三态，编辑者可直接推进工作，无需额外审批环节
- 协作总览新增逾期事项和未完成事项数量，事项卡片突出逾期、高优先级与责任人
- 新增服务端组织边界、角色权限、责任人校验、状态留痕和浏览器端回归测试

## [v4.1.0-beta.1] - 2026-08-23

### 决策审批闭环

- 编辑者可将门店、候选点或整个工作区的结论提交管理层审批
- 审批固化提交时的远端版本与工作区快照，不会被后续地图修改篡改
- admin/owner 可批准或驳回；驳回必须填写意见，提交人不能自审
- 支持提交人撤回待审记录，所有提交、批准、驳回和撤回动作进入组织审计时间线
- 协作页面新增待审 KPI、内联审批表单、状态标识和管理层处理入口
- CI 通用 Actions 升级至当前 Node 24 运行时版本，消除旧 Node 20 警告

## [v4.0.0-beta.4] - 2026-08-17

### 查看模式面板收纳

- 查看模式的右侧经营总览面板新增独立隐藏按钮
- 面板隐藏后提供“显示经营总览”毛玻璃悬浮入口，不影响顶部菜单的独立收起状态
- 经营总览收起状态保留在当前浏览器会话中；编辑模式和移动端不会显示多余恢复入口
- 浏览器回归覆盖经营总览隐藏、恢复、顶部菜单隐藏和原编辑器切换组合

## [v4.0.0-beta.3] - 2026-08-17

### 菜单视觉与顶部收纳

- 总览、经营时间、选址模型、数据中心和企业协作统一复用原地图编辑器的深色毛玻璃、边框、圆角、阴影和主色变量
- 顶部经营决策菜单新增手动隐藏按钮，隐藏后地图和各工作区自动释放顶部空间
- 新增居中的“显示顶部菜单”悬浮入口，收起状态保留在当前浏览器会话中
- 首页主操作继续使用“开始使用”，并保持进入原地图编辑器、返回决策首页的交互不变

## [v4.0.0-beta.2] - 2026-08-17

### 编辑器视觉与入口

- 首页“进入编辑”改为更清晰的“开始使用”
- 开始使用后隐藏经营决策顶部菜单和右侧洞察，地图恢复全屏
- 编辑态恢复原地图编辑器的悬浮控制区、图层面板、间距和毛玻璃视觉风格
- 新增轻量“返回决策首页”按钮，返回后恢复管理层总览和安全查看模式

## [v4.0.0-beta.1] - 2026-08-17

### 私有部署与企业协作

- 新增无第三方运行依赖的私有 Node API，使用原子 JSON 持久化作为单节点试点存储
- 新增组织初始化、scrypt 密码摘要、12 小时不透明会话及 viewer/editor/admin/owner 权限边界
- 工作区保存使用版本号和 `If-Match` 乐观锁；冲突返回 409，客户端明确要求先拉取，不静默覆盖
- 新增实体评论、邮箱 @提醒、解决/重开状态、只读快照分享、决策简报和企业品牌
- 新增文件投递/webhook 同步任务元数据与组织内审计记录
- Full 版新增企业协作工作区；公开 Pages 明确保持仅本地模式，Lite 不开放私有协作入口
- 新增私有部署、TLS、备份恢复、权限矩阵与迁移到 PostgreSQL/PostGIS 的运维边界
- 自动化覆盖密码存储、会话/分享过期、越权阻止、组织隔离、版本冲突和浏览器协作流程
